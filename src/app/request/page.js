...
const Request = () => {
    ...
    const pushToRequests = (name, artist) => {
        setResultArray([])
        setRequests([name, artist])
    }
  ...
  return (
    <div className='px-4 pt-2 font-inter'>
            ...
            <div className='w-full border-1 border-[#b1a7a6] flex flex-col items-center space-y-1 rounded-md mt-2'>
                {
                    resultArray.map((track) => (
                        <div key={track.id} className='w-full px-2 py-4 border-b border-1 border-[#b1a7a6] flex flex-row justify-between'>
                            ...
                            <button className='bg-white text-[#a4161a] underline text-xs font-bold px-2 py-4 rounded-2xl' onClick={() => pushToRequests (track.name, track.artists[0].name)}>request</button>
                        </div>
                    ))
                }
            </div> 
        ...
        }
        {
            requests.length > 0 ? (
                <div className='mt-2'>
                    <h1 className='font-inter'>{`${requests[0]} by ${requests[1]} has been sent to the Dj. Enjoy the party and anticipate your song!`}</h1>
                </div>
            ) : 
            <div></div>
